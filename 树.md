...
​	最基础的BFS：**[199. 二叉树的右视图](#199-二叉树的右视图)**、[102. 二叉树的层序遍历](#102-二叉树的层序遍历)、[103. 二叉树的锯齿形层序遍历](#103-二叉树的锯齿形层序遍历) 



### [树的前中后序遍历](#树的前中后序遍历)

​	**[94. 二叉树的中序遍历](#94-二叉树的中序遍历)**、[144. 二叉树的前序遍历](#144-二叉树的前序遍历])、**[145. 二叉树的后序遍历](#145-二叉树的后序遍历)**

​	**[105. 从前序与中序遍历序列构造二叉树](#105-从前序与中序遍历序列构造二叉树)**、[106. 从中序与后序遍历序列构造二叉树](#106-从中序与后序遍历序列构造二叉树)、[889. 根据前序和后序遍历构造二叉树](#889-根据前序和后序遍历构造二叉树)
...
**思路：**需要将子树路径的结果汇总到上面，用后续遍历。返回包含当前节点在内左右大的一侧，用于上一层的计算。在每个节点处都更新全局变量sum，最终sum结果即为最大路径和。
...
```java
    class Solution {
        ...
        private int postOrder(TreeNode root) {
            ...
            int left = Math.max(0, postOrder(root.left));  //和0比较是为了排除左右是负数，这种情况可以不要左右
            int right = Math.max(0, postOrder(root.right));
            sum = Math.max(sum, left + right + root.val);
            return root.val + Math.max(left, right);//为上一层返回子树的最大单链路
        }
    }
```
